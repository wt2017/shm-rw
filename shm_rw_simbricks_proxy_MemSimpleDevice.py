...
# Create memory proxy component
# We inherit from MemSimpleDevice so Gem5Sim can process it correctly
# The key insight: Gem5Sim looks for MemHostInterface on components it simulates
# By adding this proxy to the same simulator as the host, Gem5Sim will find it
class MemoryProxy(sys_mem.MemSimpleDevice):
    def __init__(self, s: system.System):
        super().__init__(s)
        self.name = "memory_proxy"
        # MemSimpleDevice already has everything we need (_mem_if, _addr, _size, etc.)
        # We just need to exist as a MemSimpleDevice that Gem5Sim can process

# Create the memory proxy component with the same memory region as external memory
mem_proxy = MemoryProxy(sys)
mem_proxy._addr = 0x3FC0000  # Physical address used by shm_rw
mem_proxy._size = 4 * 1024 * 1024  # 4MB
mem_proxy._as_id = 0
...
# The connect_host method will handle the interface connection properly
...
shm_rw_app.binary = "/home/ubuntu/shm_rw"
shm_rw_app.args = []
